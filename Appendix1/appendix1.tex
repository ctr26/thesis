...
\begin{align}
  ...
  \intertext{ Likewise define a projection at an angle 8, PO(t),  and its Fourier transform by}
S_{\theta}(w) =  \int_{-\infty}^{\infty} P_\theta9t) e^{-i2\pi w t} dt
\end{align}
\begin{align}
\intertext{The simplest example of the Fourier Slice Theorem is given for a projection at \(\theta = 0\). First, consider the Fourier transform of the object along
...
F(u,0) =  \int_{-\infty}^{\infty} P_{\theta = 0} (x) e^{-i2\pi(ux)} dx \label{eq:f(u,0)}
