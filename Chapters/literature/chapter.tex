...
\gls{Gaussian beam}s, however, have a distinct trade-off when used in \gls{LSFM} applications, in that the thinner the light sheet the narrower the usable field of view.
...
\begin{align}
	...
	z_R       & = \frac{\pi w_0^2}{\lambda} \label{eq:Rayleigh} \\
  w_0 & = 1.4 \lambda \frac{\gls{NA}}{n} \label{eq:spot_size}
\end{align}

Where \gls{z_R} is the \gls{Rayleigh length}; \gls{w_0} is the spot of size of the beam; \gls{lambda} is the wavelength of light and $n$ is the refractive index.
...
In mounting the orthogonal pair at an angle such that they were planar to a flat surface, Betzig~\emph{et~al.} maximised detection and excitation \gls{NA} whilst minimising sample mounting impedance.
...
Virtual sample manipulation is alluring as virtual manipulations can be performed that are impossible physically.
Zhang~\emph{et~al.}~\cite{li_axial_2014} created a virtual sample using a similar relay system to Dunsby~\emph{et~al.}, but crucially they positioned a flat mirror which rotated their virtual sample by \SI{90}{\degree}, shown in \figurename~\ref{fig:single_objectives/atomic_mirror}.
...
\begin{figure}
  ...
  \begin{subfigure}[t]{0.475\textwidth}
    ...
    \caption{A flat mirror is used to rotate virtual image~\cite{li_axial_2014}}\label{fig:single_objectives/atomic_mirror}
  \end{subfigure}
...
Bessel beams can be constructed, optically, from either an \gls{axicon} lens or an amplitude mask with an annular ring opening, the latter being inexpensive but the most optically lossy.
...
In doing so, the undesirable orders were constructively interfered to reinforce the zeroth orders of the parallel beams.
