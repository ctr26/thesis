...
Moreover, there was a three-fold increase in both tissue elasticity and viscosity associated with the mid-blastula transition.
...
\begin{figure}
 ...
 \caption[Periods of embryonal development of the \gls{zebrafish}]{\footnotemark{}
...
\end{figure}
\footnotetext{Reproduced from \cite{kimmelStagesEmbryonicDevelopment1995}}
...
\begin{table}
 ...
 \SI{72}{\hour} & Early larva & The swim bladder inflates; food-seeking behaviour occurs. \\
  \bottomrule
 \end{tabular*}
 \caption{Developmental stages of \gls{zebrafish}~\cite{kimmelStagesEmbryonicDevelopment1995}}\label{tab:zfish_dev}
\end{table}
...
When a stress \(\sigma \) is applied, the spherical rigid object, in this case a magnetic bead made of superparamagnetic nanoparticles embedded into a polyester matrix, moves and the displacement is quantified by the strain \(\epsilon \).
...
\begin{gather}
 \epsilon = \frac{1}{\gls{E_visc}} \sigma \footnote{See Appendix \ref{appendix:tweezertheory} for proof} \label{eq:linearspring}
 ...
 \intertext{Knowing that \(\epsilon = \epsilon_s + \epsilon_p\), the strain is therefore:}
...
% \begin{gather}
 ...
 E \epsilon_p + \eta_2 \dot{\epsilon_p} = 0 \implies \epsilon_p = C_2 \exp\left(-\frac{E}{\eta_2}(t-t_1)\right)
...
\intertext{The equations of motion may be summarised as}
 ...
 \frac{\sigma}{\eta_1}t_1 + \frac{\sigma}{E}\left(\exp\left( \frac{t_1}{\tau_2}\right)-1\right)\exp\left(-\frac{t}{\tau_2} \right) &\text{for } (t-t_1)\leq t_1
...
In the magnetic tweezers system a square loop of iron was used to carry magnetic flux from four solenoids to four embedded magnetic tips.
...
\begin{figure}
 ...
 \begin{subfigure}[t]{0.45\linewidth}
  \centering
  \includegraphics[width=\linewidth]{tweezer_spim_render_shooped.pdf}
  \caption{\Gls{light-sheet} objective lens placed within the magnetic tweezer chamber.}
 \end{subfigure}\quad
 \begin{subfigure}[t]{0.45\linewidth}
  \centering
  \includegraphics[width=\linewidth]{tweezer_spim_render_side.pdf}
  \caption{Side view with magnetic centre of tweezers at the double focus of the illumination and imaging objective lenses.}
 \end{subfigure}
 \caption{CAD design of the magnetic tweezer housing coupled to the light-sheet objective lenses.
 Orange denotes magnetic flux through the square iron core which in-turn feeds magnetic flux until the magnetic tweezers.}\label{fig:tweezer_spim}
\end{figure}
...
% Force calibration was realised by analysing the velocities of a diameter paramagnetic bead in known viscosity silicone oil at different combination of iythntensities of currents.
...
A single colour channel was used for both cellular fluorescence and the bead for two reasons, firstly this doubled the potential temporal resolution and therefore halved the overall photon dose to the embryo; secondly the bead was exceptionally bright in it's own channel, much more so than the embryo) and so we relied on spectral bleed-through on the dichroic mirror which reduced the fluorescence intensity to a signal comparable with the cells.
...
% This trend was seen irrespective of the starting developmental age of the embryo, which may suggest work hardening of the \gls{zebrafish}. %TODO Colin dont like %TODO swoger dnt like
...
%%
The proposed mechanical models provide a good explanation for the trajectory of the bead during the active force application and the initial recoil (\(R^2 = \SI{0.94(5)}\)). %TODO R^2
However, bead movements in the later recovery period were more erratic and were not accounted for by the model.
...
Note, experiments are shown to not terminate together as some specimens did not survive the full \SI{2}{\hour} course of the experiment.
 %
...
In fact, cell shape (measured as an assumed eclipse with the ratio of the two axes being the shape strain metric) strain rates were highly correlated with, and largely account for, tissue deformation strain rate (\figurename~\ref{fig:cell_tracking}f,	\(p\text{correlation} = 0.0001757\), \(R^2 = 0.743\)).
...
% \begin{figure}\ContinuedFloat
 ...
 (e)~-~(h): Analysis of cell area  and cell aspect ratio revealed that cells disappear from the plane of interest, suggesting that cells at the front rearranged in 3D, while at the rear, cells were stretched (e).
...
Only a single plane of interest was considered (the plane of the bead) for the image analysis, which made the analysis more tractable as the software routines that were used often failed to segment cells sensibly and required significantly more computing time for the full \gls{3D} analysis.
Cells became more compressed and moved out of the plane of interest (decreasing in area) in front of the bead and, conversely, cells became stretched and entered  the plane at the rear (\figurename~\ref{fig:cell_tracking}a-b).
...
This suggests that there may be a relatively simple cellular interpretation of the physical model measured using magnetic bead rheology. %TODO FIGURE 4J
...
\figurename~\ref{interconnected_protein_signalling} depicts the connection of protein signalling interactions with environmental effects.
