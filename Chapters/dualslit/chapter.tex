...
The rolling of the shutter with a concomitant sweeping of the beam of light allows for live confocal imaging, depicted in \figurename~\ref{fig:slit_scanning_alt}.
...
To measure this, \SI{200}{\nano\metre} fluorescent beads were suspended in \SI{1}{\percent} \gls{agarose VII} gel at a concentration of \SI{2.3e3}{\text{parts} \per\milli\litre}.
...
The contrast was calculated by fitting a \gls{2D} Gaussian to each image, with varying slit width, and plotting the fitted amplitude \(A\), from the \gls{2D} Gaussian model:
\begin{align}
...
The fastest exposure possible for each chip (10ms) provides the maximum \SI{100}{\hertz} imaging.
...
\begin{figure}
  ...
  \caption{
  Principle behind doubling the frame rate of the Orca Flash v4 when using \gls{slit-scanning} mode.
...
\subsection{Dual slits using an \gls{SLM}}
...
\subsubsection{An all optical dual-slit implementation}

An alternative implementation was employed whereby, immediately after all four colours were spatially co-aligned, a beam splitter was added with two returning mirrors (similar to Michelson interferometer).
...
The \emph{secondary} beam was pushed to the top of the image using appropriate the kinematic mirror flanking the beam splitter.
...
A similar effect to a swept beam in a \gls{light-sheet} system can be realised in a \gls{wide-field} system using an \gls{SLM}.
...
With this, the contrast and resolution of the output image increase in the direction of propagation of the shutter as simulated in \figurename~\ref{fig:widefield_slit} using equation.
\begin{align}
    I_{\text{em}}(x,y,n) = \sum_{n=0}^{n=h-1}
    \Biggl(\begin{split}
                &\left[ (s(x,y) (I_{\text{ill}}(x,y,n) * \text{PSF}(x,y))) * \text{PSF}(x,y) \right] \\ &\cdot  H(y-n)*H(-(y-n)+w_{\text{slit}})
    \end{split}\Biggr)
\end{align}
\begin{align}
...
\(n\) pixel number of the active row as the shutter rolls; \(H(y-n)*H(-(y-n)+w_{\text{slit}})\) represents the active region on the detector as the shutter rolls with a width of \(w_{\text{slit}}\) and \(\text{PSF}(x,y)\) is the point-spread function.
\begin{align}
...
\begin{align}
  \intertext{and}
    I_{\text{em}}(x,y,n) = \sum_{n=0}^{n=h-1}
    \Biggl(\begin{split}
                &\left[ (s(x,y) I_{\text{ill}}(x,y,n)) * \text{PSF}(x,y) \right] \\ &\cdot  H(y-n)*H(-(y-n)+w_{\text{slit}})
    \end{split}\Biggr)
\end{align}
As the imaging and illumination objective lens are not the same.
The application of confocal \gls{slit-scanning} was also considered for structured illumination sources.
...
\begin{figure}[h]
  ...
  \begin{subfigure}[t]{0.25\textwidth}
      ...
      \caption{Slit-scanning FFT}
  \end{subfigure}
  % \includegraphics{widefield_slit}
  \caption{Simulation of a confocal slit-scanned wide-field or swept light-sheet system.
  (a) Is the raw, image with b) showing Fourier space and the frequency passband.
  (c) Shows the confocal \gls{slit-scanning} in progress and (d) shows what the \gls{SLM} will be displaying.
  (d) is the final image as it would appear after an acquisition and e) is the the new \gls{OTF} with a wider support in \(y\).
  }
  \label{fig:widefield_slit}
\end{figure}
...
High contrast fringes are needed so that sharp peaks can be localised from the sinusoidal illumination pattern.
...
Slit-scanned \gls{SIM} simulations of a test image were successfully reconstructed as seen in \figurename~\ref{fig:sim_slit_reconstructed} with the patial illumination as:
\begin{align}
    % \intertext{For \gls{SIM} illumination:}
    I_{\text{ill}}(x,y,n) &= {I_0} [1 + \cos(k_x x + k_y y + \phi]
\end{align}
At an angle \(\theta = \frac{k_x}{k_y}\) of 3 and a phase (\(\phi\)) of 3.
% \begin{figure}
%   \centering
...
In \gls{slit-scanning} \gls{mSIM}, each exposure the \gls{SLM} will display a single row of illumination spots from the full lattice with a standard \gls{mSIM} pattern.
...
Ordinarily, the virtual pin-holing step is achieved computational by reconstructing from an \(n\times n\) (\(O(n^2\)) set of images; using \gls{slit-scanning} the reconstruction reduces from to $n$ (\(O(n)\)) operations.
Schroff~\emph{et. al.}, for instance, require \SI{200 x 200}{} images for a single \gls{mSIM} reconstruction.
The method proposed here, would therefore increase the speed of this process by \SI{200}{} fold.
...
Such image splitting devices are commercially available, with the trade-off of \gls{FOV} for time-resolution.
...
The simulations presented here can be immediately adapted onto microscope systems with \gls{SLM}s already embedded, provided there is intensity (rather then phase) control at the sample plane.
