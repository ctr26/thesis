...
\begin{align}
    ...
    P_\theta(v) = &\int \int f(x,y)\delta (x\cos(\theta)+y \sin(\theta)-v)dx dy
\end{align}

Where $P_\theta(t)$ is the Radon transform of $f(x,y)$ which represents the contrast image of \gls{2D} slice of the specimen.
...
\intertext{where $Q_\theta$ is the filtered projection data, and $f(x,y)$ is the back-projected image.}
\end{align}
%\todo{Verify this?!?!?!}
...
The blurring arises as \(Q_\theta\) is back-projected (smeared) across the image plane for each angle of reconstruction; which means that not only does the back-projection contribute at the line it is intended to (line CD in \figurename~\ref{fig:TODO}), but all other points along the back-projecting ray.
...
Imaging scenes in stereo allows for the triangulation of individual features in three dimensional space.
features or fiducials in one detector to another (When the features are uniquely identifiable points, such as fluorescent beads).
...
image data from
between two cameras. %TODO Say more?
...
Given the \gls{CCD} parameters, we can translate pixel coordinates \((u, v(\) into \gls{sample plane} coordinates \((x, y)\):
\begin{align}
    u = u_0 + k_u x \\
    v = v_0 + k_v y
\end{align}

Knowing the focal length of the imaging system, \gls{sample plane} coordinates may be translated into a ray in \gls{3D}.
The ray can be defined by a point \(\textbf{p}\) in camera-centred coordinates, where it crosses the \gls{sample plane} as:
...
Using a model of a system with two views allows for the triangulation of rays based on image correspondences, this is an important part of stereovision.
...
\subsubsection{The Essential matrix}
